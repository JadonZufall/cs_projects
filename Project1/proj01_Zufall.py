...
def read_file_data(file_name):
    ...
    for line in file_lines:
        ...
        values = [word.strip().lower() for word in values]
...
def count_questions(file_data):
    ...
    """
    # Entire function got scrapped because I was counting the values when I should have been counting the keys.
...
def get_amount_questions(number_questions):
    ...
    while not valid_amount:
        ...
        string_amount = input("How many words would you like to be quizzed on? ")

        # Validate that the number can be converted to an integer.
        try:
            question_amount = int(string_amount)
        except ValueError:
...
def get_questions_list(file_data, number_questions):
    ...
    file_data_sample = sample(file_data_keys, number_questions)

    # Return the sample taken from the file_data's keys.
    return file_data_sample


def ask_questions(file_data, questions):
    ...
    for question_number, question in enumerate(questions):
        ...
        for word_number in range(0, len_answer):
            ...
            user_input = input(f"Word [{word_number + 1}]: ")
...
def output_results(name, date, quiz_results, number_correct):
    """ Writes the results to user prompted file. """
    # Request file name from user.
    file_name = input("Enter an output file (or press enter to quit): ")
...
def main():
    ...
    file_data = read_file_data(file_name=file_name)

    # Count the number of questions in the file_data.
    number_questions = count_questions(file_data=file_data)
    ...
    question_amount = get_amount_questions(number_questions=number_questions)

    # Samples the file_data and selects random words to quiz the user on.
    questions = get_questions_list(file_data=file_data, number_questions=question_amount)

    # Ask the user the quiz questions.
    quiz_results = ask_questions(file_data=file_data, questions=questions)
